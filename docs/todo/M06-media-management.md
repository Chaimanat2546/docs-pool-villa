...
**Status:** Complete — Staging lifecycle, single-flight retry, UI refresh และ exact-target cleanup verified
...
- Post-closeout section-delete race remediation (13 สิงหาคม 2026): migration `20260813150200_docs_section_delete_race_guard.sql` และ pgTAP regression ปิดช่องว่าง prepare-empty → create/move Document/Media เข้า subtree → finalize ลบ DB เกิน manifest. Local reset/test ผ่าน; migration ใหม่นี้ยังไม่ได้ apply Staging และต้องรอภูอนุมัติโดยตรง. ไม่มี Remote/Production action ใน fix wave นี้.
