-- Docs-only: new documents append within their selected section under the
-- existing transaction-level advisory lock. Updates retain caller-provided order.
...
begin
  ...
  if v_is_create then
    ...
    values (p_id, p_section_id, btrim(p_title), p_slug, nullif(btrim(coalesce(p_excerpt, '')), ''), p_content,
      p_status, v_create_sort_order, case when p_status = 'published' then now() else null end);
  else
