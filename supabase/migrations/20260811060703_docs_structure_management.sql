-- M02 owns structure-level data rules.  These changes intentionally leave all
-- Legacy objects untouched.

alter table public.doc_sections
  add column description text,
  add column is_published boolean not null default true,
  add column created_by uuid default auth.uid(),
  add column updated_by uuid default auth.uid();

alter table public.doc_documents
  add column sort_order integer not null default 0 check (sort_order >= 0);

create index doc_documents_section_sort_order_idx
  on public.doc_documents (section_id, sort_order, id);

drop index public.doc_sections_parent_sort_order_idx;
create index doc_sections_parent_sort_order_idx
  on public.doc_sections (parent_id, sort_order, id);

create or replace function doc_private.doc_validate_section_structure()
returns trigger
language plpgsql
set search_path = pg_catalog
as $$
declare
  v_parent_parent_id uuid;
  v_route_path text;
begin
  if new.slug = any (array['admin', 'api', 'search', 'login', '_next']) then
    raise exception 'This slug is reserved.' using errcode = '23514';
  end if;

  if new.parent_id is not null then
    if new.parent_id = new.id then
      raise exception 'A section cannot be its own parent.' using errcode = '23514';
    end if;

    select parent_id
      into v_parent_parent_id
      from public.doc_sections
      where id = new.parent_id;

    if not found then
      raise exception 'The selected parent section does not exist.' using errcode = '23503';
    end if;

    if v_parent_parent_id is not null then
      raise exception 'Sections may only be nested two levels deep.' using errcode = '23514';
    end if;

    select '/' || parent.slug || '/' || new.slug
      into v_route_path
      from public.doc_sections as parent
      where parent.id = new.parent_id;
  else
    v_route_path := '/' || new.slug;
  end if;

  if exists (
    select 1
    from public.doc_route_redirects as redirect
    where redirect.old_path = v_route_path
  ) then
    raise exception 'This section route is already reserved by a redirect.' using errcode = '23505';
  end if;

  return new;
end;
$$;

create or replace function doc_private.doc_set_section_audit_fields()
returns trigger
language plpgsql
set search_path = pg_catalog
as $$
begin
  new.updated_at := now();
  new.updated_by := auth.uid();
  return new;
end;
$$;

create trigger doc_sections_validate_structure
before insert or update of parent_id, slug on public.doc_sections
for each row execute function doc_private.doc_validate_section_structure();

create trigger doc_sections_set_audit_fields
before update on public.doc_sections
for each row execute function doc_private.doc_set_section_audit_fields();

-- Replace the M01 helper so a hidden parent or section cannot be exposed by a
-- published document underneath it.
create or replace function doc_private.doc_section_is_public(p_section_id uuid)
returns boolean
language sql
security definer
set search_path = pg_catalog
as $$
  select exists (
    select 1
    from public.doc_sections as section
    left join public.doc_sections as parent on parent.id = section.parent_id
    where section.id = p_section_id
      and section.is_published
      and (parent.id is null or parent.is_published)
      and (
        exists (
          select 1
          from public.doc_documents as document
          where document.section_id = section.id
            and document.status = 'published'
        )
        or exists (
          select 1
          from public.doc_sections as child_section
          join public.doc_documents as document
            on document.section_id = child_section.id
          where child_section.parent_id = section.id
            and child_section.is_published
            and document.status = 'published'
        )
      )
  );
$$;

create or replace function public.doc_section_delete_preview(p_section_id uuid)
returns table (
  child_section_count bigint,
  document_count bigint,
  media_count bigint,
  document_titles text[]
)
language sql
security invoker
set search_path = pg_catalog
as $$
  with subtree as (
    select id from public.doc_sections where id = p_section_id
    union all
    select child.id
    from public.doc_sections as child
    where child.parent_id = p_section_id
  ), documents as (
    select document.id, document.title
    from public.doc_documents as document
    join subtree on subtree.id = document.section_id
  )
  select
    (select count(*) from subtree) - 1,
    (select count(*) from documents),
    (select count(*) from public.doc_media as media join documents on documents.id = media.document_id),
    coalesce((select array_agg(documents.title order by documents.title) from documents), array[]::text[])
  where (select doc_private.doc_is_admin());
$$;

create or replace function public.doc_delete_section(p_section_id uuid)
returns void
language plpgsql
security invoker
set search_path = pg_catalog
as $$
declare
  v_media_count bigint;
begin
  if not (select doc_private.doc_is_admin()) then
    raise exception 'Administrator access is required.' using errcode = '42501';
  end if;

  perform 1
  from public.doc_sections
  where id = p_section_id
  for update;

  if not found then
    raise exception 'The section was not found.' using errcode = 'P0002';
  end if;

  with subtree as (
    select id from public.doc_sections where id = p_section_id
    union all
    select child.id from public.doc_sections as child where child.parent_id = p_section_id
  )
  select count(*) into v_media_count
  from public.doc_media as media
  join public.doc_documents as document on document.id = media.document_id
  join subtree on subtree.id = document.section_id;

  if v_media_count > 0 then
    raise exception 'Media cleanup must complete before deleting this section.' using errcode = 'P0001';
  end if;

  with subtree as (
    select section.id, section.parent_id, section.slug
    from public.doc_sections as section
    where section.id = p_section_id
    union all
    select child.id, child.parent_id, child.slug
    from public.doc_sections as child
    where child.parent_id = p_section_id
  ), document_paths as (
    select case
      when subtree.parent_id is null then '/' || subtree.slug || '/' || document.slug
      else '/' || parent.slug || '/' || subtree.slug || '/' || document.slug
    end as path
    from public.doc_documents as document
    join subtree on subtree.id = document.section_id
    left join public.doc_sections as parent on parent.id = subtree.parent_id
  )
  delete from public.doc_route_redirects as redirect
  using document_paths
  where redirect.target_path = document_paths.path;

  delete from public.doc_documents
  where section_id = p_section_id
     or section_id in (select id from public.doc_sections where parent_id = p_section_id);

  delete from public.doc_sections where parent_id = p_section_id;
  delete from public.doc_sections where id = p_section_id;
end;
$$;

revoke all on function doc_private.doc_validate_section_structure() from public;
revoke all on function doc_private.doc_set_section_audit_fields() from public;
revoke all on function public.doc_section_delete_preview(uuid) from public, anon, authenticated, service_role;
revoke all on function public.doc_delete_section(uuid) from public, anon, authenticated, service_role;
grant execute on function public.doc_section_delete_preview(uuid), public.doc_delete_section(uuid) to authenticated;
