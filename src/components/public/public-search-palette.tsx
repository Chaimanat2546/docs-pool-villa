  ...
  return <Dialog.Root open={open} onOpenChange={setOpen}>
    ...
    <Dialog.Portal><Dialog.Backdrop className="fixed inset-0 z-40 bg-background/60 backdrop-blur-sm" />
      <Dialog.Popup aria-label="ค้นหาคู่มือ" className="fixed left-1/2 top-24 z-50 w-[min(40rem,calc(100vw-2rem))] -translate-x-1/2 rounded-xl border bg-card p-3 shadow-xl outline-none">
        <label className="sr-only" htmlFor="palette-search">ค้นหาคู่มือ</label><input ref={inputRef} id="palette-search" type="search" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="ค้นหาคู่มือ" className="h-11 w-full rounded-md bg-muted px-3 outline-none focus-visible:ring-2 focus-visible:ring-primary" />
        <ul className="mt-2 max-h-80 overflow-y-auto" aria-label="ผลการค้นหา">{items.map((item) => <li key={`${item.kind}-${item.id}-${item.href}`}><a href={item.href} className="block min-h-11 rounded-md px-3 py-2 hover:bg-muted"><span className="block font-medium">{item.title}</span>{item.kind === "heading" && <span className="text-sm text-muted-foreground">หัวข้อ: {item.heading}</span>}</a></li>)}</ul>
      </Dialog.Popup>
    </Dialog.Portal>
  </Dialog.Root>;
}
