...
import { SearchShortcut } from "@/components/public/search-shortcut";


export function PublicHeader() {
  return <><SearchShortcut /><a href="#main-content" className="skip-link">ข้ามไปยังเนื้อหา</a><header className="sticky top-0 z-30 border-b bg-background/95 backdrop-blur">
    <div className="mx-auto flex min-h-14 max-w-7xl items-center gap-4 px-4 sm:px-6">
      ...
      <form action="/search" className="ml-auto flex w-full max-w-sm items-center rounded-full border bg-card px-3">
        ...
        <kbd className="hidden rounded border px-1.5 py-0.5 text-xs text-muted-foreground sm:inline">Ctrl K</kbd>
      </form>
