...
export function SectionPanel({ selectedSectionId, mode, explorer }: SectionPanelProps) {
  ...
  const [operations, setOperations] = useState(explorer.pendingSectionOperations);
  const [deleteTarget, setDeleteTarget] = useState<AdminExplorerSection | null>(null);
  ...
  const parent = selectedSection?.parentId
    ? explorer.sections.find((section) => section.id === selectedSection.parentId) ?? null
    : null;
  const rootSections = explorer.sections.filter((section) => section.parentId === null);
  const selectedHasChildren = selectedSection
    ? explorer.sections.some((section) => section.parentId === selectedSection.id)
    : false;
  ...
  const directDocuments = selectedSection
    ? explorer.documents.filter((document) => document.sectionId === selectedSection.id)
    : explorer.documents;
  ...
  function retryOperation(operation: MediaOperationView) {
    ...
    startTransition(async () => {
      ...
      if ("pending" in result) {
        setOperations((current) => current.map((candidate) =>
          candidate.operationId === result.operation.operationId ? result.operation : candidate,
        ));
        return;
      }

      setOperations((current) => current.filter((candidate) => candidate.operationId !== operation.operationId));
      router.refresh();
    });
  }
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          {selectedSection && (
            <button
              ...
              className="min-h-11 rounded-full px-4 text-sm font-medium text-destructive hover:bg-destructive/10 disabled:opacity-50"
            >
              ลบหมวด
            </button>
          )}
        ...
        <Dialog.Portal>
          ...
          <Dialog.Popup className="fixed left-1/2 top-1/2 z-50 w-[min(32rem,calc(100vw-2rem))] -translate-x-1/2 -translate-y-1/2 rounded-xl bg-card p-6 shadow-xl outline-none">
            <div className="flex items-start gap-3">
              <FileWarning className="mt-1 text-destructive" aria-hidden="true" />
              <div>
                <Dialog.Title className="text-lg font-semibold">ยืนยันการลบหมวด</Dialog.Title>
                <Dialog.Description className="mt-1 text-sm text-muted-foreground">การลบเป็นแบบถาวร เอกสารและหมวดย่อยที่ไม่มีรูปจะถูกลบด้วย</Dialog.Description>
              </div>
            </div>
            {deletePreview && (
              <div className="mt-4 rounded-lg bg-muted p-3 text-sm">
                <p>หมวดย่อย: {deletePreview.childSectionCount} หมวด · เอกสาร: {deletePreview.documentCount} รายการ · รูป: {deletePreview.mediaCount} รูป</p>
                {deletePreview.documentTitles.length > 0 && (
                  <ul className="mt-2 list-disc pl-5 text-muted-foreground">
                    {deletePreview.documentTitles.map((title, index) => <li key={`${title}-${index}`}>{title}</li>)}
                  </ul>
                )}
                {deletePreview.mediaCount > 0 && <p className="mt-2 text-destructive">ระบบจะไม่ลบข้อมูลจนกว่าจะลบรูปจาก R2 สำเร็จ</p>}
              </div>
            )}
            {message && <p role="alert" className="mt-4 rounded-lg border border-destructive/30 bg-destructive/10 p-3 text-sm text-destructive">{message}</p>}
            <label className="mt-5 block text-sm font-medium" htmlFor="confirm-section-name">พิมพ์ “{deleteTarget?.title ?? ""}” เพื่อยืนยัน</label>
            <input
              autoFocus
            ...
            <div className="mt-6 flex justify-end gap-3">
              <Dialog.Close disabled={isPending} className="min-h-11 rounded-full px-4 text-sm hover:bg-muted disabled:opacity-50">ยกเลิก</Dialog.Close>
              <button
                type="button"
                disabled={isPending || !deleteTarget || confirmedName !== deleteTarget.title}
...
const ActionLink = forwardRef<HTMLAnchorElement, { href: string; disabled: boolean; children: React.ReactNode }>(function ActionLink({ href, disabled, children }, ref) {
  return (
    <Link
      ...
      className={`inline-flex min-h-11 items-center rounded-full border px-4 text-sm font-medium ${disabled ? "pointer-events-none opacity-50" : "hover:bg-muted"}`}
    >
      {children}
    </Link>
  );
