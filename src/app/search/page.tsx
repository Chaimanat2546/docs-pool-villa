...
export default async function SearchPage({ searchParams }: { searchParams: Promise<{ q?: string | string[]; page?: string | string[] }> }) {
  ...
  return <><PublicHeader /><main id="main-content" className="mx-auto max-w-3xl px-4 py-14 sm:px-6"><h1 className="text-3xl font-semibold">{heading}</h1><p className="mt-3 text-muted-foreground">{results.total ? `พบ ${results.total} เอกสาร` : input.query ? `ไม่พบเอกสารที่ตรงกับ “${input.query}”` : "ยังไม่มีเอกสารที่เผยแพร่"}</p>{results.items.length > 0 && <ol className="mt-6 divide-y rounded-xl border bg-card">{results.items.map((document) => <li key={document.id}><Link href={document.path} aria-label={document.title} className="block px-5 py-4 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-primary"><span className="block font-medium">{document.title}</span><span className="mt-1 block text-sm text-muted-foreground">{document.parentTitle ? `${document.parentTitle} / ` : ""}{document.sectionTitle}</span>{document.excerpt && <span className="mt-2 block text-sm text-muted-foreground">{document.excerpt}</span>}</Link></li>)}</ol>}{results.pageCount > 1 && <nav className="mt-6 flex items-center justify-between gap-4" aria-label="เปลี่ยนหน้าผลการค้นหา">{results.page > 1 ? <Link href={searchHref(input.query, results.page - 1)} className="min-h-11 content-center rounded-full border px-4 text-sm font-medium">หน้าก่อนหน้า</Link> : <span />}{results.page < results.pageCount ? <Link href={searchHref(input.query, results.page + 1)} className="min-h-11 content-center rounded-full border px-4 text-sm font-medium">หน้าถัดไป</Link> : <span />}</nav>}</main></>;
}
